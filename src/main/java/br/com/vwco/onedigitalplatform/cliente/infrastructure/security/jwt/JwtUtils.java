...
public class JwtUtils {
	...
	public String generateAccessTokenJwt(String email) {
		return Jwts.builder().setSubject(email).setIssuedAt(new Date())
				...
				.signWith(SignatureAlgorithm.HS512, jwtSecret).claim("is_access_token", true).compact();
	}
	...
	public String refreshJwtToken(String refreshToken) {
		try {
			...
			return Jwts.builder().setSubject(userDetails.getEmail()).setId(userDetails.getId().toString())
					.claim("firstName", userDetails.getUsername()).setIssuedAt(new Date())
					.setExpiration(new Date((new Date()).getTime() + refreshTokenJwtExpirationMs))
	...
	public boolean isAccessToken(String jwtToken) {
		try {
			...
			return claims.get("is_access_token", Boolean.class);
